...
const createUser = async (req, res, next) => {
  ...
  if (!doesUserExists) {
    ...
    const token = jwt.sign({ id:createdUser.id , email:createdUser.email } , process.env.JWT_SECRET_KEY , {expiresIn:'1h'});
...
const loginUser = async (req, res, next) => {
  ...
  const token = jwt.sign({id:existingUser.id , email:existingUser.email} , process.env.JWT_SECRET_KEY , {expiresIn:'1h'});
